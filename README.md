...
- `frontend/` — page HTML autonome qui consomme l'API.
...
3. Ouvrez le front-end :
   ...
   - Depuis un navigateur local, servez `frontend/index.html` (par ex. `python -m http.server 3000` à la racine du dépôt) puis accédez à `http://localhost:3000`.
   - Cliquez sur "Créer un utilisateur", ajoutez des UM puis récupérez les exercices.

## Fonctionnalités incluses

- Authentification légère en mémoire (inscription / connexion / déconnexion) pour lier la progression à un compte.
...
- Gamification légère : streak, badges dynamiques et événements analytics traçables (unit_learned, exercise_completed/failed, session_completed, seed_loaded).
